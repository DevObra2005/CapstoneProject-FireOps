...
public class PhaseTransitionManagerClassroom : MonoBehaviour
{
    // -------------------------------------------------------
    // WHAT THIS DOES:
    // Classroom-only copy of PhaseTransitionManager.
    ...
    // Kept separate from the Office version so edits here never
    // affect PhaseTransitionManager used in the Office scene.
    // -------------------------------------------------------
    ...
    public string phaseTwoScene = "Classroom3DScene1";
    ...
    void Update()
    {
        ...
        {
            Debug.LogWarning("[PhaseTransitionClassroom] HazardCounterManager.Instance is null.");
            return;
        }
        ...
        {
            Debug.LogWarning("[PhaseTransitionClassroom] DialogueManager.Instance is null.");
            return;
        }
    ...
    void HighlightAlarm()
    {
        if (alarmPanel == null)
        {
            Debug.LogWarning("[PhaseTransitionClassroom] No alarm panel assigned — skipping to extinguisher.");
        ...
        ClassroomAlarmTarget target = alarmPanel.GetComponent<ClassroomAlarmTarget>();
        if (target == null)
            target = alarmPanel.AddComponent<ClassroomAlarmTarget>();
    ...
    void HighlightExtinguisher()
    {
        ...
        {
            Debug.LogWarning("[PhaseTransitionClassroom] No fire extinguisher assigned!");
            ShowConfirmScreen();
            return;
        }
        ...
        ClassroomExtinguisherTarget target =
            fireExtinguisher.GetComponent<ClassroomExtinguisherTarget>();
        if (target == null)
            target = fireExtinguisher.AddComponent<ClassroomExtinguisherTarget>();
    ...
    void ShowConfirmScreen()
    {
        if (confirmPanel != null)
        {
            ...
            Debug.Log("[PhaseTransitionClassroom] Confirm screen shown.");
        }
        else
        {
            LoadPhaseTwo();
    ...
    public void LoadPhaseTwo()
    {
        ...
        Debug.Log("[PhaseTransitionClassroom] Loading Phase 2...");
